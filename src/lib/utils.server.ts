...
import { emailVerificationCodesTable as emailVerificationCodes } from "@/services/drizzle/schema";
...
export async function sendVerificationCode(emailAddress: string, code: string) {
	...
	// 	console.info(
	// 		`Email From: ${serverEnvs.EMAIL_FROM} | Email To: ${emailAddress}`,
	// 	);
	...
	try {
		if (serverEnvs.EMAIL_PROVIDER === "resend") {
			const resend = new Resend(serverEnvs.RESEND_API_KEY! as string);
